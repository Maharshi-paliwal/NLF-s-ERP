...
//   // Fetch branch list when component mounts (if you ever want it)
//   useEffect(() => {
...
//   const itemsArray = poData.items ? poData.items.map(item => ({
...
//   // Add this line to define additionalDetails
//   const additionalDetails = poData.additionalDetails || [];
...
const PDFClientPO = ({ show, onHide, poData }) => {
  ...
  // Fetch branch list and header images when component mounts
  useEffect(() => {
    const fetchBranchList = async () => {
      try {
        setIsLoading(true);
  ...
  // Helper function to resolve header image URL
  ...
  // Use the dynamic header image resolution
  const headerImagePath = resolveHeaderImage(officeBranch);

  // Fallback to hardcoded paths if dynamic image not found
  const getHeaderImagePath = (branch) => {
    // If we have a dynamic image, use it
    if (headerImagePath) return headerImagePath;
    
    // Otherwise fall back to hardcoded paths
    const branchMap = {
      Kolkata: "/extra/Kolkata.jpeg",
      Delhi: "/extra/Delhi.jpeg",
      Indore: "/extra/Indore.jpeg",
